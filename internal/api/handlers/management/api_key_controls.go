...
func (h *Handler) PostAPIKeyControls(c *gin.Context) {
	...
	// Persist may have written the response already; guard against a double write.
	if c.Writer.Written() {
		return
	}
...
func buildAPIKeyControlResponse(cfg *config.Config, idx int, stats *usage.RequestStatistics, includeUsage bool, recentPerModel int, maskKey bool) apiKeyControlResponse {
	...
	usageOut := &apiKeyControlUsage{
		...
		FailureCount:      snapshot.FailureCount, // Snapshot only exposes global failure — leave 0 per-key for now
	...
	// Suppress the misleading global failure count we borrowed above.
	usageOut.FailureCount = 0
